...
def draw_prof_labs( cal, f_name='total_labs.svg', prof_list=prof_list ):
    ...
    for component in cal.walk():
        if component.name == "VEVENT" and component['prof'] in prof_list and component['type'] in [ct_marker_LAB]:
            st = component.get('date_time_start'); ft = component.get('date_time_end'); prof = component.get('prof');
            np = prof_list.index( prof ); nts = lab_timespans.index( component['date_time_start'].time() ); nwd = st.weekday();
    ...
    # Вспомогательные числа
    week_starts_of_sem = sorted( set( e['date_time_start'] - timedelta( days=e['date_time_start'].weekday(), hours=e['date_time_start'].hour, minutes=e['date_time_start'].minute, seconds=e['date_time_start'].second ) for e in cal.walk() if e.name == "VEVENT" ))
    nb_weeks_in_sem = len( week_starts_of_sem )
...
def draw_prof_labs_2( cal, f_name='total_labs.svg', prof_list=prof_list, what2draw=[ct_marker_LAB] ):
    weekday_stripes = [ [list()], [list()], [list()], [list()], [list()], [list()] ] # Без воскресенья
    # Количество дней от понедельника первой до воскресенья последней
    nb_weeks = (sem_finish + timedelta(days = 7-sem_finish.weekday()) - (sem_start - timedelta(days = sem_start.weekday()) )).days / 7
    ...
    # Вспомогательные числа
    st = [ datetime.strptime( e['date_time_start'], '%Y-%m-%d %H:%M:%S') for e in cal.walk() if e.name == "VEVENT" ]
    #ft = [ datetime.strptime( e['date_time_end'], '%Y-%m-%d %H:%M:%S') for e in cal.walk() if e.name == "VEVENT" ]
    week_starts_of_sem = sorted( set( dt - timedelta( days=dt.weekday(), hours=dt.hour, minutes=dt.minute, seconds=dt.second ) for dt in st ))
    nb_weeks_in_sem = len( week_starts_of_sem )
    ...
    for nbw in range(0,nb_weeks):
        ...
        for nd, wd in enumerate( wdays[:-1] ):
            ...
            sorted_events = sorted( T[nd][nbw], key=lambda el: (el['date_time_start'],el['prof']) )
            if len( sorted_events ) > 0: h = strips_per_week[nbw] * h_strip / len( sorted_events )
            for (el_i,el) in enumerate( sorted_events ):
                st = datetime.strptime( el['date_time_start'], '%Y-%m-%d %H:%M:%S');
                df = st + timedelta( hours=3,minutes = 10 );
                #color = groups_colors[ el['group'] ]
                #color = '#fcfcfc'
                #color = "rgb(255, 255, 230)"
                color = profs_colors[ el['prof'] ]
                #if el['date_time_start'].hour == 9: color = col_9;
                #elif el['date_time_start'].hour == 13: color = col_13;
                #elif el['date_time_start'].hour == 16: color = col_16;
                ...
                tx = etree.Element( 'text', x=str(x + w/2 - 0.42*w), y=str(y + h/2 - 0.1*h_strip), fill='black', style=txt_style_2s ); tx.text = u'%s%s' % ( st.hour, s_time('%02d' % st.minute) ); doc.append( tx );
                tx = etree.Element( 'text', x=str(x + w/2 - 0.42*w), y=str(y + h/2 + 0.4*h_strip), fill='black', style=txt_style_2s ); tx.text = u'%s%s' % ( ft.hour, s_time('%02d' % ft.minute) ); doc.append( tx );
...
def draw_prof_labs_3( f_name='total_labs.svg', prof_list=prof_list ):
    ...
    # Наполнить таблицу
    for component in cal.walk():
        if component.name == "VEVENT" and component['prof'] in prof_list and component['type'] in [ct_marker_LAB] :# and component['FIRST'] == 1:
            #st = component.get('date_time_start');
            st = datetime.strptime( component['date_time_start'], '%Y-%m-%d %H:%M:%S');
            #ft = component.get('date_time_end');
            ft = datetime.strptime( component['date_time_end'], '%Y-%m-%d %H:%M:%S');
            prof = component.get('prof');
            nwd = st.weekday();
            np = prof_list.index( prof );   #nts = lab_timespans.index( component['date_time_start'].time() );
            #nw = int(floor( ((st - ss).days)/7 ));
            #groups_list.add( component['group'] );
